...
>4. Tomemos $g$ cualquiera tal que $E_{g}(g\circ X_{(n)})=0\quad\forall \theta\in \Theta$ 
...
>12. Observacion mia. Con lo cual $P_{\theta}(g(X_{(n)})=0)=P_{\theta}(\Omega)=1$ (Si no fuera asi tendria algun conjunto $A\subseteq \Omega$ de medida no nula tal que ${} g(X_{(n)}(A))\neq 0 {}$ pero $g$ es nula salvo conjunto de medida 0 Lebesgue, con lo cual $B=X_{(n)}(A)$ tiene que tener medida $0$ Lebesgue osea $P_{X_{(n)}}(B)=0$ que es lo mismo que $P_{\theta}(X_{(n)}\in B)=0$ que es absurdo)
